...
{           DelphiUIAutomation                                              }
{                                                                           }
{           Copyright 2018 JHC Systems Limited                              }
{                                                                           }
{***************************************************************************}
{                                                                           }
...
{                                                                           }
{***************************************************************************}
unit AutomatedExpandCollapsePanel;
...
type
  TAutomatedExpandCollapsePanel = class(TPanel,
        IValueProvider,
  ...
  protected
    ...
    procedure SetExpanded(const Value: Boolean); virtual; abstract;
  public
    { Public declarations }
...
procedure Register;
begin
  RegisterComponents('Automation', [TAutomatedExpandCollapsePanel]);
end;

{ TAutomatedExpandCollapsePanel }

function TAutomatedExpandCollapsePanel.Collapse: HResult;
begin
  SetExpanded(false);
  result := S_OK;
end;

function TAutomatedExpandCollapsePanel.Expand: HResult;
begin
  SetExpanded(true);
  result := S_OK;
end;

function TAutomatedExpandCollapsePanel.GetPatternProvider(patternId: SYSINT;
...
function TAutomatedExpandCollapsePanel.GetPropertyValue(propertyId: SYSINT;
...
function TAutomatedExpandCollapsePanel.GetValue: String;
begin
  result := self.Text;
end;

procedure TAutomatedExpandCollapsePanel.SetTheValue (const AValue: String);
begin
  self.SetValue(PWideChar(AValue));
end;

function TAutomatedExpandCollapsePanel.Get_ExpandCollapseState(
...
function TAutomatedExpandCollapsePanel.Get_HostRawElementProvider(
  out pRetVal: IRawElementProviderSimple): HResult;
begin
  result := UiaHostProviderFromHwnd (self.Handle, pRetVal);
end;

function TAutomatedExpandCollapsePanel.Get_IsReadOnly(out pRetVal: Integer): HResult;
begin
  pRetVal := 0;   // Maybe?
  Result := S_OK;
end;

function TAutomatedExpandCollapsePanel.Get_ProviderOptions(
...
function TAutomatedExpandCollapsePanel.Get_Value(out pRetVal: WideString): HResult;
begin
  Result := S_OK;
  pRetVal := self.GetValue;
end;

function TAutomatedExpandCollapsePanel.SetValue(val: PWideChar): HResult;
begin
  self.Text := val;
  Result := S_OK;
end;

procedure TAutomatedExpandCollapsePanel.WMGetObject(var Message: TMessage);
begin
